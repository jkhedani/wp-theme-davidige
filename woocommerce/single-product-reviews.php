...
<?php if ( comments_open() ) : ?><div id="reviews"><?php 
	...
	if ( $count > 0 ) :
		...
		echo '<h3>'.sprintf( _n('%s review for %s', '%s reviews for %s', $count, 'woocommerce'), $count, wptexturize($post->post_title) ).'</h3>';
	...
	if ( have_comments() ) : 
		...
		echo '<p class="add_review"><a href="#review_form" class="inline show_review_form button roundButton">'.__('Add Review', 'woocommerce').'</a></p>';
